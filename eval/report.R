...
sparsePlot = function(x, y, granularity=100, secondary=FALSE, col="black", title="Bee Plot", ylim=c(0,1)) {
  ...
  if (secondary) {
    par(new=TRUE)
    plot(x[c(seq(1,length(x),step),length(x))], y[c(seq(1,length(y),step), length(y))], type="l", axes=FALSE, ylab="", xlab="", bty="c", col=col, main=title, ylim=ylim)
    axis(4)
    mtext("secondary y-axis label",4) 
  } else {
    plot(x[c(seq(1,length(x),step),length(x))], y[c(seq(1,length(y),step), length(y))], type="l", col=col, main=title, ylim=ylim)
  }
}
...
sparsePlot(x=states$time, y=states$numberOfBees, granularity=granularity, title=title, ylim=c(0,max(states$numberOfBees)))
dev.off()
